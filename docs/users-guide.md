...
Vibe Coder is an idle strategy game about running a software organization. The
core loop is a conversation: the player expresses intent through policies, the
simulation interprets those policies over time, and the game speaks back
...
**Policies take effect immediately.** Changing the security allocation from 5%
to 30% will reduce CVE debt accumulation starting from the next tick. Changing
it back will reverse that effect.
...
Each category creates different incident types when it reaches a threshold. The
debt constellation view shows the current vector as a spatial map, making it
easier to spot which categories are approaching critical levels.

______________________________________________________________________

## Incidents and events

**Incidents** are probabilistic events generated from the current debt and risk
...
Choosing civic response costs time but builds brand and alignment. Every option
has trade-offs.
...
The organization advances through a series of stages, each representing a more
...
through a review screen. The risks of each stage are shown before accepting.
