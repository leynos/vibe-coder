...
- **Project scripts (Bun)**:
- `fmt`: `bunx biome format --write src tests tools scripts package.json
  biome.jsonc bunfig.toml`
- `lint`: `bunx biome ci src tests tools`
- `check:types`: `tsc --noEmit --skipLibCheck`
- `dev`: `vite`
- `build`: `vite build`
- `preview`: `vite preview`
- `tokens:build`: `bun tokens/build/style-dictionary.js`
- `test`: `bun test --preload ./tests/setup-happy-dom.ts --preload
  ./tests/setup-snapshot-guard.ts`
...
- `test:e2e:container`: `bash scripts/e2e-container.sh`
- `semantic`: `bun run semantic:lint`
- `test:all`: run all quality gates in sequence
...
- **Runtime schemas**: Validate I/O boundaries (network responses, localStorage
  payloads, URL params) with `valibot`. Generate TS types from schemas or
...
- **Biome**: One tool for format + lint. Configure with strict rules: disallow
  ...
  - Biome respects `.biomeignore` and VCS ignore files; large build trees like
    any `dist/` directory are excluded.
...
- **Translations**: Store locale files under
  `public/locales/<lang>/common.ftl` using Mozilla Fluent format and load
  strings through the Fluent backend.
...
- **Playwright**: Run `@axe-core/playwright` scans, exercise keyboard
  ...
  translations. Use `bun test:e2e` (dispatches to host or container via
  `scripts/e2e.sh`) or `bun run test:e2e:container` to run inside a Podman
  container on Fedora/WSL2.
